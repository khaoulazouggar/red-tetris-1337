	...
	*/
	...
	leaveRoom = (io, socket, rooms, players) => {
		return new Promise((resolve, reject) => {
			...
			socket.leave(room);
			io.to(room).emit('chat', { message: `${playerremoved.name} Left the room`, type: 'joined' })
			const player = players.filter(plyr => plyr.socketId === socket.id)
			...
			if (Admin && playersinRoom.length > 0) {
				...
				io.to(room).emit('chat', { message: `${playersinRoom[0].name} is the Admin now`, type: 'joined' })
				resolve(true)
			}
			else {
